...
export const Sidebar: React.FC<SidebarProps> = ({ activeTool, setActiveTool, activePanel, setActivePanel, onGenerateVariation }) => {
  ...
  const creativeTools = [
    ...
    { id: 'gradient', icon: Rainbow, label: 'Gradient (G)' },
  ...
  return (
    <div className="w-14 bg-zinc-950 border-r border-zinc-800 flex flex-col items-center py-4 gap-2 shrink-0 z-10 overflow-y-auto custom-scrollbar">
      {tools.map((tool) => (
        <button
          key={tool.id}
          onClick={() => setActiveTool(tool.id as ToolType)}
          className={`
            w-10 h-10 rounded flex items-center justify-center transition-all group relative shrink-0
            ${activeTool === tool.id 
      ...
      <div className="w-8 h-px bg-zinc-800 my-2 shrink-0" />
      ...
      <button 
        ...
        className="w-10 h-10 rounded flex items-center justify-center text-zinc-500 hover:text-purple-400 hover:bg-purple-900/20 transition-colors group relative animate-pulse-slow shrink-0"
      ...
      <div className="mt-auto flex flex-col gap-4 pt-4">
        <button 
          ...
          className={`w-10 h-10 rounded flex items-center justify-center transition-colors group relative shrink-0 ${activePanel === 'library' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-200 hover:bg-zinc-900'}`}
        ...
        <button 
          ...
          className={`w-10 h-10 rounded flex items-center justify-center transition-colors group relative shrink-0 ${activePanel === 'layers' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-200 hover:bg-zinc-900'}`}
          title="Layers"
        >
        ...
        <button 
          ...
          className={`w-10 h-10 rounded flex items-center justify-center transition-colors group relative shrink-0 ${activePanel === 'colors' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-200 hover:bg-zinc-900'}`}
          title="Colors"
        >
