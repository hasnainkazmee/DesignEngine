...
import { AlertTriangle, CheckCircle2, Info, ArrowRight, Activity, Heart, Wind, ShieldCheck, Sparkles, ChevronDown, ChevronUp, Droplet, Type, Layout } from 'lucide-react';
...
export const ValidationPanel: React.FC<ValidationPanelProps> = ({ issues, vitalSigns, intention, onSetIntention, onFix, projectName }) => {
  ...
  const VitalBar = ({ label, value, color }: { label: string, value: number, color: string }) => (
      <div className="mb-2">
          <div className="flex justify-between text-[10px] text-zinc-500 mb-0.5 font-mono uppercase tracking-wide">
              <span>{label}</span>
              <span>{value}%</span>
          </div>
  ...
  if (showCertificate) {
      return (
          <div className="fixed inset-0 z-[100] bg-zinc-950 text-white flex items-center justify-center font-serif animate-in fade-in duration-700">
               <div className="max-w-2xl w-full border-y border-zinc-800 py-12 px-8 text-center relative bg-zinc-900/20">
                   ...
                   <div className="flex justify-center gap-4">
                       <div className="h-12 border border-zinc-800 w-32 flex items-center justify-center text-[10px] text-zinc-600 font-mono">
                           SIG: AI-992
                       </div>
                       <div className="h-12 border border-zinc-800 w-32 flex items-center justify-center text-[10px] text-zinc-600 font-mono">
  ...
  return (
    <div className={`border-t border-zinc-800 flex flex-col bg-zinc-950 transition-all duration-300 ease-in-out ${isCollapsed ? 'h-10' : 'h-1/2 min-h-[300px]'}`}>
      {/* Header */}
      <div 
        className="px-4 py-2 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/50 cursor-pointer hover:bg-zinc-900"
        onClick={() => setIsCollapsed(!isCollapsed)}
      >
         <div className="flex items-center gap-2">
            <Activity size={14} className={issues.length > 0 ? "text-yellow-500" : "text-emerald-500"} />
            <h2 className="text-xs font-bold uppercase tracking-wider text-zinc-400">
               Design Vital Signs
            </h2>
            <span className="text-[10px] text-zinc-600 font-mono px-1.5 py-0.5 bg-zinc-900 rounded border border-zinc-800">
         ...
         <div className="flex items-center gap-3">
             <div className="relative group" onClick={(e) => e.stopPropagation()}>
                <button className="text-[10px] font-bold text-zinc-400 hover:text-white bg-zinc-800/50 px-2 py-0.5 rounded flex items-center gap-1 transition-colors">
                    {intention} <ChevronDown size={10} />
                </button>
                <div className="absolute right-0 bottom-full mb-1 w-32 bg-zinc-900 border border-zinc-800 rounded shadow-xl z-50 hidden group-hover:block">
            ...
            {isCollapsed ? <ChevronUp size={14} className="text-zinc-600"/> : <ChevronDown size={14} className="text-zinc-600"/>}
         </div>
      </div>
      ...
      <div className={`flex-1 overflow-y-auto custom-scrollbar ${isCollapsed ? 'hidden' : 'block'}`}>
         ...
         <div className="p-4 border-b border-zinc-900">
             <div className="flex items-end justify-between mb-4">
                 <div>
                     <div className="text-[10px] text-zinc-500 uppercase tracking-widest mb-1">Health</div>
             ...
             <VitalBar label="Heartbeat" value={vitalSigns.heartbeat} color="bg-emerald-500" />
             <VitalBar label="Breathing" value={vitalSigns.breathing} color="bg-blue-500" />
             <VitalBar label="Integrity" value={vitalSigns.integrity} color="bg-purple-500" />
         </div>
            ...
            <div className="px-2 py-1 text-[10px] font-bold text-zinc-600 uppercase">Oracle Insights</div>
             {issues.length === 0 ? (
                <div className="p-6 text-center text-zinc-600">
                    ...
                    <p className="text-xs italic">"Harmony achieved."</p>
                </div>
             ) : (
                 issues.map((issue) => (
                     <div key={issue.id} className="bg-zinc-900 border border-zinc-800 p-3 rounded-sm group hover:border-zinc-700 transition-colors relative overflow-hidden pl-3">
                         <div className={`absolute left-0 top-0 bottom-0 w-0.5 ${issue.category === 'Soul' ? 'bg-purple-500' : issue.severity === 'warning' ? 'bg-yellow-500' : issue.severity === 'info' ? 'bg-zinc-500' : 'bg-blue-500'}`} />
                                 ...
                                 {issue.fixAction && issue.fixValue && (
                                     <div className="mt-2 flex items-center justify-between">
                                         <div className="text-[10px] text-zinc-500">
                                            Suggestion: <span className="text-zinc-300">{String(issue.fixValue).replace('#','')}</span>
                                         </div>
                                         {issue.autoFixAvailable && (
                                             <button 
                                                onClick={() => onFix(issue.id)}
                                                className="bg-blue-900/30 hover:bg-blue-900/50 text-blue-300 text-[10px] px-2 py-1 rounded flex items-center gap-1 transition-colors border border-blue-500/20"
                                             >
